
const Quotes = {

    one: 
        {quote: 'The purpose of our lives is to be happy -Dalai Lama-'},
    two: 
        {quote: "Many of life's failures are people who did not realize how close they were to success when they gave up -THOMAS A EDISON-"},
    three:
        {quote: 'Not how long, but how well you have lived is the main thing -SENECA-'},
    four: 
        {quote: 'Everything negative, pressure, challenges, is all an opportunity for me to rise. -KOBE BRYANT-'},
    five: 
        {quote: 'You never really learn much from hearing yourself speak. -GEORGE CLOONEY'},
    six: 
        {quote: 'The healthiest response to life is joy. -DEEPAK CHOPRA-'},
    seven: 
        {quote: 'Keep calm and carry on -Wintson Churchill-'},
    eight: 
        {quote: 'If you pray for rain you got to deal with the mud too -DENZEL WASHINGTON-'},
    nine: 
        {quote: 'Maybe that’s what life is… a wink of the eye and winking stars.” — Jack Kerouac'},
    ten: 
        {quote: 'Old man’s gotta be the old man, fish has gotta be the fish. You gotta be who you are in this world, no matter what. -DENZEL WASHINGTON-'},

    eleven: 
        {quote: 'Get Busy living or get busy dying -Stephen King-'},
    twelve: 
        {quote: 'In order to write about life, first you must live it -Ernest Hemingway-'},
        
    thirteen: 
        {quote: 'Discipline makes you grow. Motivation keeps you going. That is Consistency'}

}
export default Quotes;

